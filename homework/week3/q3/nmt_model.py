...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ###     self.decoder (LSTM Cell with bias. Do NOT use torch.nn.lstm but use the one you implemented in 3.3-(b).)
        ...
        self.decoder = LSTMCell_assignment(input_size=embed_size+hidden_size, hidden_size=hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        x = self.model_embeddings.source(source_padded)     # embed source sentences
        packed_x = pack_padded_sequence(x, source_lengths) # pack source sentences
        enc_hiddens_packed, (last_hidden, last_cell) = self.encoder(packed_x) # encode source sentences
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens_packed) # pad packed source sentences
        enc_hiddens = enc_hiddens.permute(1, 0, 2) # permute source sentences
        init_decoder_hidden = torch.cat([last_hidden[0], last_hidden[1]], dim=1) # concatenate forward and backward hidden states
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        dec_hidden, _ = self.decoder(Ybar_t, dec_state)
        enc_hiddens_proj_t = enc_hiddens_proj.permute(0,2,1)
        e_t = torch.bmm(dec_hidden.unsqueeze(1), enc_hiddens_proj_t).squeeze(1)
        ### END OF YOUR CODE
        ...
        tanh=nn.Tanh()
        alpha_t=softmax(e_t)
        if(alpha_t.shape[0]!=enc_hiddens.shape[0]):
            alpha_t=torch.unsqueeze(alpha_t,0)
